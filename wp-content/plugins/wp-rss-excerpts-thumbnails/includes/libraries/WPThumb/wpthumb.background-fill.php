...
class WP_Thumb_Background_Fill {
	...
	private function fill_color( array $colors ) {
		...
		if ( $current_size['width'] != $size['width'] ) {
			...
			$colorToPaint = imagecolorallocatealpha( $new_image, substr( $colors['right'], 0, 3 ) + 0, substr( $colors['right'], 3, 3 ) + 0, substr( $colors['right'], 6, 3 ) + 0, substr( $colors['left'], 9, 3 ) + 0 );
		...
		if ( $current_size['height'] != $size['height'] ) {
			...
			$colorToPaint = imagecolorallocatealpha( $new_image, substr( $colors['bottom'], 0, 3 ) + 0, substr( $colors['bottom'], 3, 3 ) + 0, substr( $colors['bottom'], 6, 3 ) + 0, substr( $colors['left'], 9, 3 ) + 0 );
